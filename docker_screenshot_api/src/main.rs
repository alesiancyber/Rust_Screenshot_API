...
async fn screenshot_handler(
    service: web::Data<Arc<DockerScreenshotService>>,
    req: web::Json<ScreenshotRequest>,
) -> impl actix_web::Responder {
    match service.ensure_container_running().await {
        Ok(_) => {
            match service.take_screenshot(req.into_inner()).await {
                Ok(screenshot) => {
                    HttpResponse::Ok()
                ...
                Err(e) => {
                    ...
                    HttpResponse::InternalServerError().body("Screenshot failed")
                }
            }
        }
        Err(e) => {
            eprintln!("Container error: {}", e);
            HttpResponse::InternalServerError().body("Container management failed")
        }
    }
}
...
async fn main() -> Result<()> {
    println!("Starting Docker Screenshot API...");

    // Create screenshot service
    let screenshot_service = Arc::new(DockerScreenshotService::new(
        "screenshot-chrome",
        "http://localhost:4444",
        "screenshots/docker",
    ));
